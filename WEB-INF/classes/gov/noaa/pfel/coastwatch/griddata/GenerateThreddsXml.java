...
public class GenerateThreddsXml {
  ...
  public static StringArray generateThreddsXml(
    ...
    for (String name : twoNames) {
      ...
      for (String fourName : fourNames) {
        ...
        if (fgdc != null) {
          ...
          for (String s : fgdcArray) {
            if (s.startsWith(summaryStart)) {
              summary = s.substring(summaryStart.length());
              break;
            }
          }
        }
        ...
        // e.g., hday or 1day
        for (String timePeriodInFileName : timePeriods) {
    ...
    catalog.append(incompleteMainCatalogText, 0, insertDatasetsHerePo);
    catalog.append(
        """
                <!-- BEGIN INFO GENERATED BY GenerateThreddsXml. -->
    ...
    if (categoryShortTitles[cat].size() > 0) {
      ...
      for (int i = 0; i < categoryShortTitles[cat].size(); i++) {
        ...
        if (!newTwoLetter.equals(oldTwoLetter)) {
          ...
          if (!oldTwoLetter.isEmpty()) // not for the first HFR dataset
          catalog.append("      </dataset>\n");
      ...
      Test.error(
          """
                      ...
                      It requires 4 parameters:
                       ...
                       'dataSubDir' the subdirectory of dataMainDir,\s
                           e.g., satellite/, which has subdirectories
                           like AT/ssta/1day with .nc data files.
                           Only the dataSubDir (not the dataMainDir) is used for the\s
                       ...
                       'incompleteMainCatalog' is the full name of the incomplete main catalog.xml
                           ...
                           and HF Radar dataset tags will be inserted. For example,\s
                           /usr/local/tomcat/content/thredds/GeneratedXml/incompleteMainCatalog.xml
                       'xmlMainDir' the directory which will be created (if necessary) to hold\s
                           ...
                           subdirectories with the new dataset catalog.xml files.\s
                           Datasets with 'C' as the first letter of the twoName
                           are assumed to be HF Radar datasets and their catalog.xml files are\s
                           stored in the Hfradar/aggreghfradar'twoName' subdirectory of xmlMainDir.
                           All other datasets\s
                           are assumed to be Satellite datasets and their catalog.xml files are\s
