...
/**
 * TableWriterJsonl provides a way to write a table to JSON (https://www.json.org/) file in a JSON
 * Lines (https://jsonlines.org/) format in chunks so that the whole table doesn't have to be in
 * memory at one time. This is used by EDDTable. The outputStream isn't obtained until the first
 * call to write().
 *
...
public class TableWriterJsonl extends TableWriter {
   ...
   * @param tWriteKVP if true, this writes colName=value. If false, this writes just the values (the
   *     "Better than CSV" example at https://jsonlines.org/examples/)
   ...
   *     String2.isVariableNameSafe. ! I think jsonp never makes sense for jsonl output, which isn't
   *     one json object.
   */
  public TableWriterJsonl(
      int tLanguage,
      EDD tEdd,
      String tNewHistory,
  ...
  public void finish() throws Throwable {
    ...
    // diagnostic
    if (verbose)
      String2.log("TableWriterJsonl done. TIME=" + (System.currentTimeMillis() - time) + "ms\n");
  }
