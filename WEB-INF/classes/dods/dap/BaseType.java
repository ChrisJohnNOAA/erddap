 ...
 * data types. All of the DAP type classes (<code>DFloat64</code>, <code>DArray</code>, etc.)
 * subclass it or one of its two abstract descendents, <code>DVector</code> or <code>DConstructor
 * </code>.
 *
 * <p>These classes and their methods give a user the capacity to set up sophisticated data types.
 * They do <em>not</em> provide sophisticated ways to access and use this data. On the server side,
 * in many cases, the class instances will have no data in them at all until the <code>serialize
 * </code> method is called to send data to the client. On the client side, most DODS application
 * programs will unpack the data promptly into whatever local data structure the programmer deems
 * the most useful.
 *
 * <p>Descendents of this class should implement the <code>ClientIO</code> interface. That interface
 * defines a <code>deserialize</code> method used by a DODS client to retrieve the variable's
 * declaration and value(s) from a DODS server.
 *
...
public abstract class BaseType implements Cloneable {
  ...
  /**
   * Returns a clone of this <code>BaseType</code>. A deep copy is performed on all data inside the
   * variable.
   *
   ...
   * Simple types (example: DFloat32) will return a single value. DConstuctor and DVector types will
   * be flattened. DStrings and DURL's will have double quotes around them.
   */
  ...
  */
   ...
   *     projection information. <strong>This is only used by Server side code.</strong>
   * @see DDS
   */
  ...
  /**
   * Print the variable's declaration. Same as <code>printDecl(os, "    ", true)</code>.
   *
   * @param os The <code>PrintWriter</code> on which to print the declaration.
   * @see DDS#print(PrintWriter)
   */
   ...
   *
   * @param os The <code>OutputStream</code> on which to print the declaration.
   * @param space Each line of the declaration will begin with the characters in this string.
   *     Usually used for leading spaces.
   * @param print_semi a boolean value indicating whether to print a semicolon at the end of the
   *     declaration.
  ...
  /**
   * Checks for internal consistency. This is important to check for complex constructor classes.
   * For example, an <code>DInt32</code> instance would return false if it had no name defined. A
   * <code>DGrid</code> instance might return false for more complex reasons, such as having Map
   * arrays of the wrong size or shape.
   *
   * <p>This method is used by the <code>DDS</code> class, and will rarely, if ever, be explicitly
   * called by a DODS application program. A variable must pass this test before it is sent, but
   * there may be many other stages in a retrieve operation where it would fail.
   *
   * @param all For complex constructor types ( <code>DGrid</code>, <code>DSequence</code>, <code>
  ...
  /**
   * Constructs a new <code>PrimitiveVector</code> object optimized for the particular data type of
   * this <code>BaseType</code>. For example, a <code>DByte</code> class would create a new <code>
   * BytePrimitiveVector</code> in this call. This allows for a very optimized, yet type-safe,
   * implementation of <code>DVector</code> functionality. For non-primitive types, such as <code>
   * DArray</code>, <code>DGrid</code>, <code>DSequence</code>, and <code>DStructure</code>, the
   * default implementation returns a <code>BaseTypePrimitiveVector</code> object which can
