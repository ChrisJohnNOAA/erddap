 ...
 * </code>. It has a type, and contains either a <code>Vector</code> of <code>String</code>s
 * containing the attribute's values, or a reference to an <code>AttributeTable</code>, if the
 * <code>Attribute</code> is a container. An <code>Attribute</code> may also be created as an alias
 * pointing to another <code>Attribute</code> of any type, including container.
 *
...
public class Attribute implements Cloneable {
   ...
   * @param value the initial value of this attribute. Use the <code>appendValue</code> method to
   *     create a vector of values.
   * @param check if true, check the value and throw AttributeBadValueException if it's not valid;
   *     if false do not check its validity.
  ...
  /**
   * Construct an <code>Attribute</code> with the given type and initial value. Checks the value of
   * the attribute and throws an exception if it's not valid.
   *
   * @param type the type of attribute to create. Use one of the type constants defined by this
   *     class.
   * @param name the name of the attribute.
   ...
   *
   * @param aliasedTo the name of the target <code>Attribute</code>. This is necessary because
   *     <code>Attribute</code> doesn't know its own name (this is handled by the <code>DAS</code>
   *     or <code>AttributeTable</code> holding it.
   ...
   *
   * @param type the type of attribute to create. Use one of the type constants defined by this
   *     class, other than <code>CONTAINER</code>.
   * @exception IllegalArgumentException thrown if <code>type</code> is <code>CONTAINER</code>. To
   *     construct an empty container attribute, first construct and empty AttributeTable and then
  ...
  /**
   * Returns a clone of this <code>Attribute</code>. A deep copy is performed on all attribute
   * values.
   *
   ...
   * @param check if true, check the validity of he attribute's value, if false don't.
   * @exception AttributeBadValueException thrown if the value is not a legal member of type
   */
   ...
   * @exception AttributeBadValueException if the value is not a legal member of type
   */
  ...
  private static final boolean checkDouble(String s) {
    ...
    // if (String2.isDoubleTrouble(s)) return true;
    try {
      Double.valueOf(s);
      return true;
    } catch (NumberFormatException e) {
