...
public class UIntArray extends PrimitiveArray {
   ...
   * @param packedValue the already packed value to be added to the array
   */
  public void addNPacked(int n, final int packedValue) {
    if (n == 0) return;
    if (n < 0)
      throw new IllegalArgumentException(
  ...
  public void addN(final StructureData sd, final String memberName, int count) {
    addNPacked(count, sd.getScalarInt(memberName));
  }
  ...
  public PrimitiveArray addFromPA(final PrimitiveArray otherPA, int otherIndex, final int nValues) {
    ...
    // add from same type
    if (otherPA instanceof UIntArray uia) {
      if (otherIndex + nValues > uia.size)
        throw new IllegalArgumentException(
            String2.ERROR
      ...
      System.arraycopy(uia.array, otherIndex, array, size, nValues);
      size += nValues;
      if (uia.getMaxIsMV()) maxIsMV = true;
      return this;
    }
