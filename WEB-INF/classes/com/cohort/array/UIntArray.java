...
public class UIntArray extends PrimitiveArray {
  ...
  private static final java.lang.foreign.ValueLayout.OfInt LAYOUT =
      java.lang.foreign.ValueLayout.JAVA_INT.withOrder(java.nio.ByteOrder.nativeOrder());
  ...
  public PrimitiveArray makeSignedPA() {
    ...
    if (wrappedArray != null) {
      ...
      java.lang.foreign.MemorySegment.copy(
          array, (0) * 4L, java.lang.foreign.MemorySegment.ofArray(ar), (0) * 4L, (size) * 4L);
    }
  ...
  public void ensureCapacity(final long minCapacity) {
    ...
    if (currentCapacity < minCapacity) {
      int newCapacity =
          PanamaHelper.calculateNewCapacity(currentCapacity, minCapacity, "UIntArray");
  ...
  protected void appendNccsvElement(final StringBuilder sb, final int i) {
    sb.append(unpack(getArrayVal(i))).append("ui");
  }
  ...
  public void reorder(final int rank[]) {
    int[] newArray = PanamaHelper.reorder(rank, size, wrappedArray, array, LAYOUT, "UIntArray");
  ...
  public void reverseBytes() {
    for (int i = 0; i < size; i++) {
      int val = array.getAtIndex(LAYOUT, i);
      array.setAtIndex(LAYOUT, i, Integer.reverseBytes(val));
    }
  }
  ...
  public void rawAppend(final PrimitiveArray pa) {
    ...
    if (pa instanceof UIntArray ua) {
      ...
      for (int i = 0; i < otherSize; i++)
        setArrayVal(
            size + i, pack(Math2.roundToUInt(pa.getRawDouble(i)))); // this DOESN'T convert mv's
    }
  ...
  public PrimitiveArray makeIndices(final IntArray indices) {
    ...
    if (wrappedArray != null) {
      for (int i = 0; i < size; i++) {
        tempUnique[i] = unpack(wrappedArray[i]);
      }
    } else {
      for (int i = 0; i < size; i++) {
        tempUnique[i] = unpack(getArrayVal(i));
      }
    }
    ...
    for (int i = 0; i < size; i++) {
      ...
      long unpackedVal = unpack(val);
      int rank = Arrays.binarySearch(tempUnique, 0, nUnique, unpackedVal);
      indices.add(rank);
    }

    long[] uniqueResult = new long[nUnique];
    System.arraycopy(tempUnique, 0, uniqueResult, 0, nUnique);
    return new UIntArray(uniqueResult);
  }
  ...
  public int[] getNMinMaxIndex() {
    ...
    if (wrappedArray != null) {
      for (int i = 0; i < size; i++) {
        final int val = wrappedArray[i];
        if (maxIsMV && val == PACKED_MAX_VALUE) {
        } else {
          long v = unpack(val);
          n++;
          if (v <= tmin) {
  ...
  public void changeSignedToFromUnsigned() {
    for (int i = 0; i < size; i++) {
      ...
      setArrayVal(
          i, (int) (i2 < 0 ? i2 + MAX_VALUE + 1 : i2 - MAX_VALUE - 1)); // order of ops is important
    }
  }
}
