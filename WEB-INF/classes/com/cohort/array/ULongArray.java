...
public class ULongArray extends PrimitiveArray {
  ...
  public PrimitiveArray subset(
    ...
    if (pa instanceof ULongArray la) {
   ...
   * @param packedValue the already packed value to be added to the array
   */
  public void addNPacked(int n, final long packedValue) {
    if (n == 0) return;
    if (n < 0)
      throw new IllegalArgumentException(
  ...
  public void addN(final StructureData sd, final String memberName, int count) {
    addNPacked(count, sd.getScalarLong(memberName));
  }
  ...
  public PrimitiveArray addFromPA(final PrimitiveArray otherPA, int otherIndex, final int nValues) {
    ...
    // add from same type
    if (otherPA instanceof ULongArray ula) {
      if (otherIndex + nValues > ula.size)
        throw new IllegalArgumentException(
            String2.ERROR
      ...
      System.arraycopy(ula.array, otherIndex, array, size, nValues);
      size += nValues;
      if (ula.getMaxIsMV()) maxIsMV = true;
      return this;
    }
