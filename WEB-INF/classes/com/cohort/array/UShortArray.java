...
public class UShortArray extends PrimitiveArray {
  ...
  private static final java.lang.foreign.ValueLayout.OfShort LAYOUT =
      java.lang.foreign.ValueLayout.JAVA_SHORT.withOrder(java.nio.ByteOrder.nativeOrder());
   ...
   * (short)getArrayVal(i)</tt>. Thus negative short values become large positive char values. Note
   * that the cohort 'missingValue' of a CharArray is different from the missingValue of a
   * UShortArray and this method does nothing special for those values. This method does nothing
   * special for the missingValues. 'capacity' and 'size' will equal ca.size. See
   * CharArray.decodeFromUShortArray().
   *
   * @param ca CharArray
   */
  public static UShortArray fromCharArrayBytes(final CharArray ca) {
    ...
    java.lang.foreign.MemorySegment.copy(
        java.lang.foreign.MemorySegment.ofArray(sarray), 0, sa.array, 0, size * 2L);
    return sa;
  }
  ...
  public PrimitiveArray makeSignedPA() {
    ...
    if (wrappedArray != null) {
      ...
      java.lang.foreign.MemorySegment.copy(
          array, (0) * 2L, java.lang.foreign.MemorySegment.ofArray(ar), (0) * 2L, (size) * 2L);
    }
  ...
  public void addN(final int n, final int value) {
    ...
    if (wrappedArray != null) {
      ...
      for (int i = size; i < size + n; i++) {
        array.setAtIndex(LAYOUT, i, pack(value));
      }
    }
    size += n;
  }
  ...
  public void ensureCapacity(final long minCapacity) {
    ...
    if (currentCapacity < minCapacity) {
      int newCapacity =
          PanamaHelper.calculateNewCapacity(currentCapacity, minCapacity, "UShortArray");
  ...
  public int indexOf(final int lookFor, final int startIndex) {
    ...
    if (wrappedArray != null) {
      for (int i = startIndex; i < size; i++) if (wrappedArray[i] == packedLookFor) return i;
    } else {
  ...
  public int lastIndexOf(final int lookFor, final int startIndex) {
    ...
    final short packedLookFor = pack(lookFor);
    if (wrappedArray != null) {
      for (int i = startIndex; i >= 0; i--) if (wrappedArray[i] == packedLookFor) return i;
    } else {
    ...
    if (wrappedArray != null && other.wrappedArray != null && maxIsMV == other.maxIsMV) {
      for (int i = 0; i < size; i++) {
        if (wrappedArray[i] != other.wrappedArray[i]) {
          return "The two UShortArrays aren't equal: this["
              + i
              + "]="
              + unpack(wrappedArray[i])
              + "; other["
              + i
              + "]="
              + unpack(other.wrappedArray[i])
              + ".";
        }
      }
    } else {
      for (int i = 0; i < size; i++) {
        if (getArrayVal(i) != other.getArrayVal(i)
            || (getArrayVal(i) == PACKED_MAX_VALUE && maxIsMV != other.maxIsMV)) {
          return "The two UShortArrays aren't equal: this["
              + i
              + "]="
              + getInt(i)
  ...
  protected void appendNccsvElement(final StringBuilder sb, final int i) {
    sb.append(unpack(wrappedArray != null ? wrappedArray[i] : getArrayVal(i))).append("us");
  }
  ...
  public void sort() {
    ...
    int which = 0;
    if (wrappedArray != null) {
      while (which < size && wrappedArray[which] < 0) which++;
    } else {
  ...
  public void reorder(final int rank[]) {
    short[] newArray = PanamaHelper.reorder(rank, size, wrappedArray, array, LAYOUT, "UShortArray");
  ...
  public void reverseBytes() {
    for (int i = 0; i < size; i++) {
      short val = array.getAtIndex(LAYOUT, i);
      array.setAtIndex(LAYOUT, i, Short.reverseBytes(val));
    }
  }
  ...
  public void internalizeFromDODS(final DataInputStream dis) throws java.io.IOException {
    ...
    for (int i = 0; i < nValues; i++)
      setArrayVal(size++, (short) dis.readInt()); // yes, ints; see above
  }
  ...
  public void rawAppend(final PrimitiveArray pa) {
    ...
    if (pa instanceof UShortArray ua) {
      ...
      for (int i = 0; i < otherSize; i++)
        setArrayVal(
            size + i, pack(Math2.narrowToUShort(pa.getRawInt(i)))); // this DOESN'T convert mv's
    }
  ...
  public PrimitiveArray makeIndices(final IntArray indices) {
    ...
    for (int i = 0; i < size; i++) {
      ...
      while (low <= high) {
        int mid = (low + high) >>> 1;
        int midVal = tempUnique[mid] & 0xFFFF;
        int targetVal = val & 0xFFFF;
        if (midVal < targetVal) {
          low = mid + 1;
    ...
    for (int i = 0; i < nUnique; i++) {
      uniqueResult[i] = tempUnique[i] & 0xFFFF;
    }
    return new UShortArray(uniqueResult);
  }
  ...
  public int switchFromTo(final String tFrom, final String tTo) {
    ...
    if (wrappedArray != null) {
      for (int i = 0; i < size; i++) {
        if (wrappedArray[i] == packedFrom) {
          wrappedArray[i] = packedTo;
  ...
  public int[] getNMinMaxIndex() {
    ...
    if (wrappedArray != null) {
      for (int i = 0; i < size; i++) {
        short rawVal = wrappedArray[i];
        if (maxIsMV && rawVal == PACKED_MAX_VALUE) {
        } else {
          int v = unpack(rawVal);
          n++;
          if (v <= tmin) {
