...
public class ByteArray extends PrimitiveArray {
   ...
   */
  public ByteArray(final byte[] anArray) {
    if (anArray == null) {
  ...
  public ByteArray(int first, int last) {
    ...
    array = java.lang.foreign.MemorySegment.ofArray(wrappedArray);
    for (int i = 0; i < size; i++) setArrayVal(i, (byte) (first + i));
  }
  ...
  public static ByteArray fromFile(final String fileName) throws Exception {
    ...
    try (final InputStream stream = File2.getDecompressedBufferedInputStream(fileName)) {
      ...
      while (available > 0) {
        ...
        if (ba.wrappedArray != null) {
          ba.size += stream.read(ba.wrappedArray, ba.size, available);
        } else {
  ...
  public PrimitiveArray subset(
    ...
    if (stopIndex < startIndex) return pa == null ? new ByteArray(new byte[0]) : pa;

    int willFind = strideWillFind(stopIndex - startIndex + 1, stride);
    ByteArray da = null;
    if (pa == null) {
      da = new ByteArray(willFind, true);
    } else {
      da = (ByteArray) pa;
  ...
  public void addN(final int n, final byte value) {
    ...
    if (wrappedArray != null) {
      ...
      for (int i = size; i < size + n; i++) {
        array.setAtIndex(java.lang.foreign.ValueLayout.JAVA_BYTE, i, value);
      }
    }
    size += n;
  }
  ...
  public byte get(final int index) {
    ...
    return array.getAtIndex(java.lang.foreign.ValueLayout.JAVA_BYTE, index);
  }
  ...
  public void set(final int index, final byte value) {
    ...
    array.setAtIndex(java.lang.foreign.ValueLayout.JAVA_BYTE, index, value);
  }
  ...
  /*    public ByteArray append(BigDecimal bdar[]) {
          ...
          ensureCapacity(size + (long)n);
          for (int i = 0; i < n; i++)
              setArrayVal(size + i, bdar[i] == null? Byte.MAX_VALUE :
