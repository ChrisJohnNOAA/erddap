...
public class String2Tests {
  @Test
  void testFindUrl() {
    int[] results = LinkHelper.findUrl("No url here. But there are multiple sentences!");
    assertEquals(-1, results[0]);
    assertEquals(-1, results[1]);

    results = LinkHelper.findUrl("https://this is definitely not valid URL! \"{><}\"");
    assertEquals(-1, results[0]);
    assertEquals(-1, results[1]);

    // check a basic whole string url
    results = LinkHelper.findUrl("http://www.example.com");
    assertEquals(0, results[0]);
    assertEquals(22, results[1]);
    results = LinkHelper.findUrl("http:\\\\www.example.com");
    assertEquals(0, results[0]);
    assertEquals(22, results[1]);
    results = LinkHelper.findUrl("https://www.example.com");
    assertEquals(0, results[0]);
    ...
    results = LinkHelper.findUrl("http://192.168.31.18/thredds/dodsC/satellite/QS/ux10/1day");
    assertEquals(0, results[0]);
    assertEquals(57, results[1]);

    // check for no protocol (but a www)
    results = LinkHelper.findUrl("www.something.com");
    assertEquals(0, results[0]);
    assertEquals(17, results[1]);

    // check for . within a path
    results = LinkHelper.findUrl("https://tds.hycom.org/thredds/dodsC/GLBu0.08/expt_90.9.html");
    assertEquals(0, results[0]);
    assertEquals(59, results[1]);
    results = LinkHelper.findUrl("https://doi.org/10.25921/RE9P-PT57");
    assertEquals(0, results[0]);
    assertEquals(34, results[1]);

    // check for ? within a fragment
    results =
        LinkHelper.findUrl("https://data.cencoos.org/#search?type_group=all&query=hab&page=1");
    assertEquals(0, results[0]);
    ...
    // gopher
    results = LinkHelper.findUrl("gopher://sdf.org/1/users/YOUR-USERNAME/cgi-bin/ls.cgi");
    assertEquals(0, results[0]);
    assertEquals(53, results[1]);
    results = LinkHelper.findUrl("gopher:\\\\sdfeu.org/1/users/YOUR-USERNAME/ls.cgi?date");
    assertEquals(0, results[0]);
    assertEquals(52, results[1]);

    // File
    results = LinkHelper.findUrl("file://some/example/file/path.ext");
    assertEquals(0, results[0]);
    assertEquals(33, results[1]);
    results = LinkHelper.findUrl("file:\\\\other\\file\\path\\with.jpg");
    assertEquals(0, results[0]);
    assertEquals(31, results[1]);

    // telnet
    results = LinkHelper.findUrl("telnet://host.edu/");
    assertEquals(0, results[0]);
    assertEquals(18, results[1]);
    results = LinkHelper.findUrl("telnet:\\\\user:pass@sub.host.edu:8080");
    assertEquals(0, results[0]);
    assertEquals(36, results[1]);
    results = LinkHelper.findUrl("telnet://user@host.edu:8080");
    assertEquals(0, results[0]);
    ...
    // smtp
    results = LinkHelper.findUrl("smtp:\\\\user:pass@sub.host.edu:8080");
    assertEquals(0, results[0]);
    assertEquals(34, results[1]);
    results = LinkHelper.findUrl("smtp://user@host.edu");
    assertEquals(0, results[0]);
    assertEquals(20, results[1]);

    // ftp
    results = LinkHelper.findUrl("ftp:\\\\user:pass@sub.host.edu:8080\\some\\file\\path");
    assertEquals(0, results[0]);
    assertEquals(48, results[1]);
    results = LinkHelper.findUrl("ftp://user@host.edu/some/other/file.ext");
    assertEquals(0, results[0]);
    assertEquals(39, results[1]);

    // smb
    results = LinkHelper.findUrl("smb:\\\\user:pass@sub.host.edu:8080\\some\\file\\path");
    assertEquals(0, results[0]);
    assertEquals(48, results[1]);
    results = LinkHelper.findUrl("smb://user@host.edu/some/other/file.ext");
    assertEquals(0, results[0]);
    assertEquals(39, results[1]);
  }
  ...
  void testFindUrl_complex() {
    ...
    int[] results = LinkHelper.findUrl("http://cencoos.org/, https://www.axiomdatascience.com");
    assertEquals(0, results[0]);
    assertEquals(19, results[1]);

    results =
        LinkHelper.findUrl("https://marinescience.ucdavis.edu/ and https://cordellbank.noaa.gov/");
    assertEquals(0, results[0]);
    assertEquals(34, results[1]);
    ...
    results = LinkHelper.findUrl("Find the url that is www.somewhere.com in this sentence.");
    assertEquals(21, results[0]);
    assertEquals(38, results[1]);

    results =
        LinkHelper.findUrl(
            "Find the url that is (inside a www.parenthetical.com) in this sentence.");
