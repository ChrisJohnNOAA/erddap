...
class NcHelperTests {
  ...
  void testGetPrimitiveArrayWithSection() throws Exception {
    ...
    try (NetcdfFile ncfile = NetcdfFiles.open(file)) {
      ...
      Test.ensureTrue(paBounded.size() < presVar.getSize(), "Bounded PA size is strictly less than full variable size");

      // CHAR section auto rank expansion test (passing 1D section for 2D char var PLATFORM_NUMBER(N_PROF=1, DATE_TIME_STRING_LENGTH=8))
      ...
      Test.ensureEqual(platformVar.getDataType(), DataType.CHAR, "PLATFORM_NUMBER is DataType.CHAR");
      ucar.ma2.Section charSectionSubRank = new ucar.ma2.Section("0:0");
      Test.ensureEqual(charSectionSubRank.getRank(), platformVar.getRank() - 1, "Section rank is rank - 1");
